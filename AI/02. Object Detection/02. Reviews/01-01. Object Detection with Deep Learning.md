...
- 객체 감지 프레임워크
  ...
  - ### Region Proposal Based Framework
    ...
    - **FPN**
      ...
      ![image-20210315145121662](C:\Users\devLupin\AppData\Roaming\Typora\typora-user-images\image-20210315145121662.png)
  ...
  - ### Regression/Classification Based Framework
    ...
    - **Pioneer Works**
      ...
      - regression-based MultiBox
        - 클래스에 구애받지 ㅇ낳는 BB 좌표 예측을 위해 여러 구성 요소의 localization과 신뢰도를 편향시키기 위해 통합 손실 도입
    ...
    - **YOLO**
      ...
      - YOLO의 아이디어
        ...
        2. 각 그리드 셀 B는 BB 및 BB의 신뢰 점수 예측
           ...
           $$
           \text {Pr(Object)}*\text {IOU}^{\text {truth}}_{\text {pred}}
           \ \\
           (Pr(Object)≥0 )\ : \ 예측에 \ 대한 \ 확신
           $$
           ...
           $$
           \text {Pr}(\text {Class}_{i}|\text {Object})
           $$
           ...
           - 테스트 시간에 각각의 box에 대한 클래스 별 신뢰도 점수
             ...
             $$
             \begin{align*}&\hspace {-2pc}{\rm Pr(Object)*IOU_{pred}^{truth}*Pr(Class_{i}|Object)} \\&\qquad \qquad \qquad \qquad \qquad \qquad ={\rm Pr(Class_{i})*IOU_{pred}^{truth}}\tag{5}\end{align*}
             $$

           - 훈련 중 손실 함수가 최적화 됨.

           $$
           \begin{align*}&\hspace{-0.7pc}\lambda _{\mathrm{ coord}}\sum _{i=0}^{S^{2}}\sum _{j=0}^{B}{\mathsf{1}}_{ij}^{\mathrm{ obj}} [(x_{i}-\hat {x_{i}})^{2}+(y_{i}-\hat {y_{i}})^{2}] \\[-2pt]&+\,\lambda _{\mathrm{ coord}}\sum _{i=0}^{S^{2}}\sum _{j=0}^{B} {\mathsf{1}}_{ij}^{\mathrm{ obj}}\left [{\left ({\sqrt {w_{i}}-\sqrt {\hat {w_{i}}})^{2}+(\sqrt {h_{i}}-\sqrt {\hat {h_{i}}}~}\right)^{2}}\right] \\[-2pt]&+\sum _{i=0}^{S^{2}}\sum _{j=0}^{B}{\mathsf{1}}_{ij}^{\mathrm{ obj}}(C_{i}-\hat {C_{i}})^{2} \\[-2pt]&+\,\lambda _{\mathrm{ noobj}}\sum _{i=0}^{S^{2}}\sum _{j=0}^{B}{\mathsf{1}}_{ij}^{\mathrm{ noobj}}(C_{i}-\hat {C_{i}})^{2} \\[-2pt]&+\sum _{i=0}^{S^{2}}{\mathsf{1}}_{i}^{\mathrm{ obj}}\sum _{c\in \text {classes}}(p_{i}(c)-\hat {p}_{i}(c))^{2}.\tag{6}\end{align*}
           \\ \ \\ \
           (x_{i}, y_{i}) : 특정\ 셀에서\ 그리드\ 셀의\ 경계에\ 대한\ 상대적인\ 상자의 중심\ 높이
           \ \\
           (w_{i}, h_{i}) : 이미지\ 크기에\ 상대적인\ 표준화\ 된\ 너비와\ 높이
           \ \\
           C_{i} : 신뢰도 \ 점수
           \ \\
           {\mathsf{1}}_{i}^{\mathrm{ obj}} : 객체의 \ 존재
           \ \\
           {\mathsf{1}}_{ij}^{\mathrm{ obj}} : 예측이\ j번째\ BB\ predictor에\ 의해\ 수행
           \ \\
           $$
        ...
        ![image-20210315164905541](C:\Users\devLupin\AppData\Roaming\Typora\typora-user-images\image-20210315164905541.png)
    ...
    - **SSD**
      ...
      - SSD 아키텍처
        ...
        ![image-20210315165317167](C:\Users\devLupin\AppData\Roaming\Typora\typora-user-images\image-20210315165317167.png)
